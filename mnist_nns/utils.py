...
def generate_batched_data(data, label, batch_size=32, shuffle=False, seed=None):
    ...
    if shuffle:
        ...
        data, label = [item[0] for item in data], [item[1] for item in data]

    batched_data = [np.array(data[i: min(i+ batch_size, len(data))]) for i in range(0, len(data), batch_size)]

    batched_label = [np.array(label[i: min(i+ batch_size, len(label))]) for i in range(0, len(label), batch_size)]
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    plt.plot(valid_loss_history, label = "Valid loss history")
    plt.legend()
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
    plt.show()
    plt.plot(train_acc_history, label = "Train acc history")
    plt.plot(valid_acc_history, label = "Valid acc history")
    plt.xlabel("Epochs")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.show()
